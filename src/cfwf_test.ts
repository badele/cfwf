...
import { DEFAULTOPTIONS, getMaxWidth, searchMarker } from "./utils.ts";
...
const comment = "this is a description";
...
test("dataset big title", async () => {
  const title = "this is a very very very very big title";
  ...
  assertEquals(194, maxwidth);
});

test("dataset generated title", async () => {
...
test("only tablename", () => {
  ...
  // const formated = await samples.outputCFWF(false);
  // const lines = formated.content.split("\n");
  // const beginheadermarker = searchMarker(lines, chartabletop || "");
});
...
test("columns and headers columns size from generated_by_sh.cfwf", async () => {
  ...
  assertEquals(124, yamlmarkerpos);
